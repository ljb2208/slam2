...
MatcherNew::MatcherNew(parameters param, Matches* matches) : param(param) {
...
void MatcherNew::pushBack (uint8_t *I1,uint8_t* I2,int32_t* dims,const bool replace) {
...
void MatcherNew::matchFeatures(int32_t method, slam2::Matrix *Tr_delta) {
...
void MatcherNew::bucketFeatures(int32_t max_features,float bucket_width,float bucket_height) {

  p_matched_p->bucketFeatures(max_features, bucket_width, bucket_height);
}

float MatcherNew::getGain (vector<int32_t> inliers) {
...
void MatcherNew::nonMaximumSuppression (int16_t* I_f1,int16_t* I_f2,const int32_t* dims,vector<Matches::maximum> &maxima,int32_t nms_n) {
...
inline void MatcherNew::computeDescriptor (const uint8_t* I_du,const uint8_t* I_dv,const int32_t &bpl,const int32_t &u,const int32_t &v,uint8_t *desc_addr) {
...
void MatcherNew::saveSmallDescriptor (bool left, const int32_t &u,const int32_t &v,const int32_t &c,uint8_t *desc_addr)
{
  std::shared_ptr<Matches::p_match> match = p_matched_p->getMatchbyMaxima(u, v, c, false);
  ...
  if (left)
  {
    match->ref1.setPatch(desc_addr);
  }
  else
  {
    match->ref2.setPatch(desc_addr);
  }
}

bool MatcherNew::getSmallDescriptor (bool left, const int32_t &u,const int32_t &v,const int32_t &c,uint8_t *desc_addr)
{
  std::shared_ptr<Matches::p_match> match = p_matched_p->getMatchbyMaxima(u, v, c, false);
  ...
  Matches::refpatch ref;
  if (left)
  {
    if (!match->ref1.initialized)
      return false;
  ...
  desc_addr[8] = ref.d9;
  desc_addr[9] = ref.d10;
  desc_addr[10] = ref.d11;
  desc_addr[11] = ref.d12;
  desc_addr[12] = ref.d13;
  desc_addr[13] = ref.d14;
  desc_addr[14] = ref.d15;
  desc_addr[15] = ref.d16;

  return true;
}

inline void MatcherNew::computeSmallDescriptor (const uint8_t* I_du,const uint8_t* I_dv,const int32_t &bpl,const int32_t &u,const int32_t &v,uint8_t *desc_addr) {
...
void MatcherNew::computeDescriptors (uint8_t* I_du,uint8_t* I_dv,const int32_t bpl,std::vector<Matches::maximum> &maxima, bool rightImage) {
...
inline uint8_t MatcherNew::saturate (int16_t in) {
  if (in<0)   return 0;
  if (in>255) return 255;
  return in;
}

void MatcherNew::filterImageAll (uint8_t* I,uint8_t* I_du,uint8_t* I_dv,int16_t* I_f1,int16_t* I_f2,const int* dims) {
...
void MatcherNew::filterImageSobel (uint8_t* I,uint8_t* I_du,uint8_t* I_dv,const int* dims) {
...
void MatcherNew::getHalfResolutionDimensions(const int32_t *dims,int32_t *dims_half) {
  dims_half[0] = dims[0]/2;
  dims_half[1] = dims[1]/2;
  dims_half[2] = dims_half[0]+15-(dims_half[0]-1)%16;
}

uint8_t* MatcherNew::createHalfResolutionImage(uint8_t *I,const int32_t* dims) {
  int32_t dims_half[3];
...
void MatcherNew::computeFeatures (uint8_t *I,const int32_t* dims,int32_t* &max1,int32_t &num1,int32_t* &max2,int32_t &num2,uint8_t* &I_du,uint8_t* &I_dv,uint8_t* &I_du_full,uint8_t* &I_dv_full, bool rightImage) {
...
void MatcherNew::computePriorStatistics (int32_t method) {
  ...
  vector<MatcherNew::delta> *delta_accu = new vector<MatcherNew::delta>[bin_num];
  
  // fill bin accumulator
  MatcherNew::delta delta_curr;
  ...
  for (int32_t v_bin=0; v_bin<v_bin_num; v_bin++) {
    for (int32_t u_bin=0; u_bin<u_bin_num; u_bin++) {
      ...
      if (delta_accu[v_bin*u_bin_num+u_bin].size()>0) {
        ...
        for (vector<MatcherNew::delta>::iterator it=delta_accu[v_bin*u_bin_num+u_bin].begin();
...
void MatcherNew::createIndexVector (int32_t* m,int32_t n,vector<int32_t> *k,const int32_t &u_bin_num,const int32_t &v_bin_num) {
...
inline void MatcherNew::findMatch (int32_t* m1,const int32_t &i1,int32_t* m2,const int32_t &step_size,vector<int32_t> *k2,
...
void MatcherNew::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
    ...
    for (i1c=0; i1c<n1c; i1c++) {
      ...
      if (i1c2==i1c) {
        ...
        if (u1p>=u2p && u1c>=u2c) {
          ...
          // add match if this pixel isn't matched yet
          if (*(M+getAddressOffsetImage(u1c,v1c,dims_c[0]))==0) {
            p_matched.push_back(MatcherNew::p_match(u1p,v1p,i1p,u2p,v2p,i2p,
                                                 u1c,v1c,i1c,u2c,v2c,i2c));
            *(M+getAddressOffsetImage(u1c,v1c,dims_c[0])) = 1;
          }
...
void MatcherNew::removeOutliersNCC()
{
    int removeCount = 0;
    int removeDispCount = 0;
    float fMin, fMax;
    ...
    for (int i=0; i < p_matched_p->inlierMatches.size(); i++)
    {
      ...
      if (dispx < param.outlier_disp_tolerance)
      {
          removeDispCount++;
    ...
    printf("NCC max: %f min: %f outliers: %i disp outliers: %i \n", fMax, fMin, removeCount, removeDispCount);
}

void MatcherNew::removeOutliers (int32_t method) {
...
bool MatcherNew::parabolicFitting(bool left, const uint8_t* I1_du,const uint8_t* I1_dv,const int32_t* dims1,
                               const uint8_t* I2_du,const uint8_t* I2_dv,const int32_t* dims2,
                               const float &u1,const float &v1, const int32_t &c1,
                               float       &u2,float       &v2, const int32_t &c2,
  ...
  if (!getSmallDescriptor(left, u1, v1, c1, desc_buffer))
  {
    computeSmallDescriptor(I1_du,I1_dv,dims1[2],(int32_t)u1,(int32_t)v1,desc_buffer);
    saveSmallDescriptor(left, u1, v1, c1, desc_buffer);
...
void MatcherNew::relocateMinimum(const uint8_t* I1_du,const uint8_t* I1_dv,const int32_t* dims1,
                              const uint8_t* I2_du,const uint8_t* I2_dv,const int32_t* dims2,
...
void MatcherNew::refinement (int32_t method) {
  ...
  for (int i=0; i < p_matched_p->inlierMatches.size(); i++)
  {
    ...
    if (method==2) {
      if (param.refinement==2) {
        if (!parabolicFitting(true, I1p_du_fit,I1p_dv_fit,dims_p,I1c_du_fit,I1c_dv_fit,dims_c,
                              it->u1p,it->v1p,it->max1.c,it->u1c,it->v1c,it->max1.c,At,AtA,desc_buffer))
          continue;
      } else {
    ...
    if (method==1 || method==2) {
      if (param.refinement==2) {
        if (!parabolicFitting(false, I2p_du_fit,I2p_dv_fit,dims_p,I2c_du_fit,I2c_dv_fit,dims_c,
                              it->u2p,it->v2p,it->max2.c,it->u2c,it->v2c,it->max2.c,At,AtA,desc_buffer))
          continue;
      } else {
...
float MatcherNew::mean(const uint8_t* I,const int32_t &bpl,const int32_t &u_min,const int32_t &u_max,const int32_t &v_min,const int32_t &v_max) {
  float mean = 0;
...
float MatcherNew::calculateNCC(const uint8_t* img1, const uint8_t* img2, const int32_t u1, const int32_t v1, const int32_t u2, const int32_t v2, const int32_t imageWidth, const int32_t imageHeight)
{
  int32_t sumI = 0;
  int32_t sumT = 0;
  float sumISq = 0;
...
float MatcherNew::correlationValue( float SumI, float SumISq, float SumIT, float SumT, float cPixels, float fDenomExp )
{
...
uint8_t MatcherNew::readPixel(const uint8_t* image, int width, int height,  int x, int y )
{
    if ( x < 0 ) x = 0;
